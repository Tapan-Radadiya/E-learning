...
} from "../services/course_module.service"
...
const getCustomSegmentFileData = async (req: Request, res: Response) => {
    const { moduleId } = req.params
    ...
    try {
        ...
        const data = await customUserSegmentService(moduleId)

        return res.type('application/vnd.apple.mpegurl').send(data)
    } catch (error) {
        console.log('error->', error)
        res.status(500).json(ApiResult({ message: "Internal Server Error" }))
        return
